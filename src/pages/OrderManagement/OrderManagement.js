...
_id: "",})
  ...
  getOrderListing(state.orderStatus)}else{
    getOrderListing()
  }
    // getOffers();
    // fetchCategoryList();
  ...
  const requestSearch = (searchedVal) => {
    ...
    const filteredRows = searchedData.filter((row) => {
      ...
      let merchantName = row.merchant.firstName+" "+row.merchant.lastName;
      let agentName = row.agent.firstName+" "+row.agent.lastName;
      // let productName = row.item;
      // let mobileNumber = JSON.stringify(get(row, "mobileNumber", ""));
      // let unitId = JSON.stringify(get(row, "sequenceId", ""));
      return (
        orderId.toLowerCase().includes(searchedVal.target.value.toLowerCase()) ||
        userName.toLowerCase().includes(searchedVal.target.value.toLowerCase()) ||
        merchantName.toLowerCase().includes(searchedVal.target.value.toLowerCase())||
        agentName.toLowerCase().includes(searchedVal.target.value.toLowerCase())
      );
    });
  ...
  const getOrderListing=async(orderStatus="ongoing")=>{
    setIsLoading(true)
    try {
      const {data}= await axios.get(`/admin/orderListing?orderStatus=${orderStatus}`)
          ...
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                ...
                /> */}
                <DashHeading>Order Management</DashHeading>
              </MenuAndBack>
                        ...
                        <div style={{ display: "flex", gap: "10px", justifyContent: "center", marginTop: "15px" }}>
                          ...
                          <span
                            ...
                            onClick={() => {
                              setStartDate(null);
                              setEndDate(null);
                              getOrderListing(orderStatusonPageNavigate);
                              // getDeliverBoysList();
                            }}
                          >
                            {" "}
                      ...
                      {recordsAfterPagingAndSorting().map((category, index) => (
                        <TableRow key={category._id}>
                          ...
                          <TableCell className={classes.textMiddle}>
                            <div>{dateOfJoining(category.createdAt)},{  moment(category.createdAt).format("hh:mm a")}</div>
                          </TableCell>
                          <TableCell className={classes.textMiddle}>
                            <div>{get(category, "orderId", "N/A")}</div>
                          </TableCell>
                          <TableCell className={classes.textMiddle} style={{ textTransform: "capitalize" }}>
                            <div>{get(category, "user.sequenceId", "N/A")},{get(category, "user.firstName", "N/A")}&nbsp;{get(category, "user.lastName", "N/A")}</div>
                          </TableCell>
                          <TableCell className={classes.textMiddle}>
                          <div>{get(category, "merchant.sequenceId", "N/A")},{get(category, "merchant.shop.name", "N/A")}</div>
                          </TableCell>

                          <TableCell className={classes.textMiddle}>
                          <div>{get(category, "agent.sequenceId", "N/A")},{get(category, "agent.firstName", "N/A")}&nbsp;{get(category, "agent.lastName", "N/A")}</div>
                          </TableCell>
                          <TableCell className={classes.textMiddle} >
                            <div style={{display:"flex",justifyContent:"center",alignItems:"center",flexWrap:"wrap",padding:"2px"}}>
                            <div>{get(category, "addressData.addressLine1", "N/A")}</div>
                            <Button
                                // variant="outlined"
                                aria-label="add"
                                ...
                                onClick={() => {
                                  setAddressDisplayData({
                                    addressLine1: category.addressData.addressLine1,
                                  
                                    addressLine2: category.addressData.addressLine2,
                                    state: category.addressData.state,
                                    district: category.addressData.district,
                                    landmark: category.addressData.landmark,
                                    pincode:category.addressData.pincode,
                                    alternatePhone: category.addressData.alternatePhone,
                                    _id: category.addressData._id,
                                  })
                          ...
                          <TableCell className={classes.textMiddle}>
                            <div>{get(category, "orderAmount", "N/A")}</div>
                          </TableCell>
                          ...
                          <TableCell className={classes.tableFlex}>
                            <div    style={{padding:"6px"}}>
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  history.push({
                                    pathname: `/adminPanel/order_details_ongoing/${category._id}`,
                                    state: [{ page: page, rowsPerPage: rowsPerPage, location: history.location.pathname,orderStatus:orderStatusonPageNavigate }],
                                  });
                                }}
                              >
                                <VisibilityIcon color="primary" />
