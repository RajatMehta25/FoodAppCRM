...
const OfferManagement = ({ history, setUsers, userData }) => {
  ...
  const getCategoryList = async () => {
    try {
      const { data } = await axios.get(`/admin/listCategories?id=${id}`);
  ...
  const handleSubmit = async (values) => {
    ...
    if (!id) {
      ...
      agentData = {
        _id: id,
        name: values.name,
        icon: values.file||values.editIcon,
      };
      ...
      <div>
        <DashboardContainer>
          <DashboardWrapper>
            <DashboardHeading>
              <MenuAndBack>
                <DashHeading>{id ? `Edit Category` : `Add Category`}</DashHeading>
              </MenuAndBack>
            ...
            <Paper className={classes.paperTableHeight} style={{ overflow: "hidden", height: "100%" }}>
              ...
              <Formik
                ...
                enableReinitialize
                initialValues={{
                  name: get(CategoryData, "name", ""),
                  //  icon: get(CategoryData, "icon", ""),
                  icon:"",
                   editIcon: get(CategoryData, "icon", ""),
                    ...
                    <Form style={{ margin: "2rem 5rem 0 5rem" }}>
                      <div className="signup-cont">
                        <div className="row">
                          ...
                          <div className="col-md-12">
                            <label className={classes.offerLabel}>Icon</label>
                            <Field name="icon">
                              {({ field }) => (
                                <div className="pb-2 mt-1">
                                  <Input
                                    ...
                                    // variant="outlined"
                                    // value={formikBag.values.icon}
                                    onChange={async(e) => {
                                      let data= await handleImageUpload(e.target.files[0])
                                      console.log("ssslslslls",data)
                                      // formikBag.setFieldValue("icon", e.target.files[0]);
                                      formikBag.setFieldValue("file", data);
                                    }}
                              ...
                              >
                                Upload Image
                                  ...
                                  {console.log("image",formikBag.values.file)}
                                 {id&&formikBag.values.editIcon!==""&&formikBag.values.file===""? <img src={formikBag.values.editIcon} alt="image"  style={{height:"100px",width:"100px",margin:"10px 0px"}} />: <img src={formikBag.values.file} alt="image" style={{height:"100px",width:"100px",margin:"10px 0px"}}/>}
                                </div>
                              )}
                            </Field>
                          </div>
